...
public class TransactionDAO {
	...
	/*
	 * public void delete(TransactionBean transaction) { Session session =
	 * HibernateUtil.getSessionFactory().openSession(); try {
	 * session.beginTransaction(); session.delete(transaction);
	 * session.getTransaction().commit(); } catch (Exception e) { if
	 * (session.getTransaction() != null) { session.getTransaction().rollback();
	...
	public TransactionBean[] getTransactionsByCustomerId(int customerId) {
		Session session = HibernateUtil.getSessionFactory().openSession();
		Query query = session
				.createQuery("from TransactionBean where customerBean_customerId = :customerId");
	...
	public TransactionBean[] getByStatus(String status) {
		Session session = HibernateUtil.getSessionFactory().openSession();
		Query query = session
				.createQuery("from TransactionBean where status = :status");
