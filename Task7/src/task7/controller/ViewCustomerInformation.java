...
public class ViewCustomerInformation extends Action {
	...
	public String perform(HttpServletRequest request) {
		...
		try {
			...
			if (form.getButton() == null || form.getCustomerEmail() == null
				...
				for (int i = 0; i < positionBeans.length; i++) {
					...
					fundInfoBeans[i].setFundId(positionBeans[i].getFundBean()
							.getFundId());
				...
				TransactionBean[] transactionBeans = transactionDAO
						.getTransactionsByCustomerId(customerBean
								.getCustomerId());
				...
				for (int i = 0; i < transactionBeans.length; i++) {
					...
					transactionHistoryBeans[i]
							.setExecuteDate(transactionBeans[i]
									.getExecuteDate());
					transactionHistoryBeans[i].setFundBean(transactionBeans[i]
							.getFundBean());
					transactionHistoryBeans[i]
							.setTransactionId(transactionBeans[i]
									.getTransactionId());
					transactionHistoryBeans[i]
					...
					String type = transactionHistoryBeans[i]
							.getTransactionType();
					String status = transactionHistoryBeans[i].getStatus();

					if (type.startsWith("D")
							|| type.startsWith("R")
							|| (type.startsWith("Buy") && status
									.equals("Pending"))) {
						transactionHistoryBeans[i]
								.setAmount(transactionBeans[i].getAmount() / 100.0);
						...
						transactionHistoryBeans[i]
								.setShares(transactionBeans[i].getAmount() / 1000.0);
						...
						transactionHistoryBeans[i].setSharePrice(price / 100.0);
						transactionHistoryBeans[i].setShares(amount * 1.0
								/ price);
					}
				}
